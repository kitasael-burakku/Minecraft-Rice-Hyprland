        ...
        const extractScript = `
            ...
            for file in "$THUMBS"/*; do
                if [ -f "$file" ]; then
                    ...
                    # Saltar thumbs de VIDEO (prefijo 000_): son posters JPEG con
                    # extensión de video (.mp4/.mkv/...). Si se le pasan a magick
                    # con su extensión real, ImageMagick puede invocar su delegate
                    # de ffmpeg pensando que es un video, y ese ffmpeg se queda
                    # colgado esperando stdin para siempre (proceso zombie).
                    case "$filename" in
                        000_*) continue ;;
                    esac
                    ...
                    if [ $found -eq 0 ]; then
                        # Prefijo 'jpg:' fuerza a magick a tratar el archivo como
                        # JPEG sin importar su extensión (defensa adicional).
                        hex=$($CMD "jpg:$file" -modulate 100,200 -resize "1x1^" -gravity center -extent 1x1 -depth 8 -format "%[hex:p{0,0}]" info:- 2>/dev/null | grep -oE '[0-9A-Fa-f]{6}' | head -n 1)
                        if [ -n "$hex" ]; then
