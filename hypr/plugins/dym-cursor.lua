-- Dynamic cursors — inclina/rota/estira el cursor segun el movimiento,
-- y magnifica al agitarlo (shake to find).
--
-- Plugin externo, no forma parte de Hyprland: github.com/virtcode/hypr-dynamic-cursors
-- Se instala con hyprpm, que guarda el .so compilado en
-- /var/cache/hyprpm/$USER/ (de root: por eso `add`/`update` piden sudo;
-- `reload` no, el .so es legible por todos).
--
-- Apagarlo entero = comentar el require("plugins.dym-cursor") de hyprland.lua.

-- ── Guarda ───────────────────────────────────────────────────────────────────
if not (hl.plugin and hl.plugin.dynamic_cursors) then
    return
end
